...
local Request = require("neopilot.request")
local Mark = require("neopilot.ops.marks")
local editor = require("neopilot.editor")
local RequestStatus = require("neopilot.ops.request_status")
local Window = require("neopilot.window")
local make_clean_up = require("neopilot.ops.clean-up")
...
local function fill_in_function(context, opts)
  ...
  local virt_line_count = context._neopilot.ai_stdout_rows
  if virt_line_count >= 0 then
    context.marks.function_location = Mark.mark_func_body(buffer, func)
  end

  local request = Request.new(context)
  local full_prompt = context._neopilot.prompts.prompts.fill_in_function()
  local additional_prompt = opts.additional_prompt
  if additional_prompt then
    full_prompt =
      context._neopilot.prompts.prompts.prompt(additional_prompt, full_prompt)

    local rules = Agents.find_rules(context._neopilot.rules, additional_prompt)
  ...
  local request_status = RequestStatus.new(
    250,
    context._neopilot.ai_stdout_rows,
    "Loading",
  ...
  request:start({
    ...
    on_complete = function(status, response)
      ...
      if status == "failed" then
        if context._neopilot.display_errors then
