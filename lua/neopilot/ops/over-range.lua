...
local function over_range(context, range, opts)
  ...
  local display_ai_status = context._neopilot.ai_stdout_rows > 1
  local top_status = RequestStatus.new(
    250,
    context._neopilot.ai_stdout_rows or 1,
    "Implementing",
    top_mark
  )
  ...
  local full_prompt = context._neopilot.prompts.prompts.visual_selection(range)
