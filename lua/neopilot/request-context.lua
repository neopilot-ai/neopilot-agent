...
--- @class _neopilot.RequestContext
...
--- @field marks table<string, _neopilot.Mark>
--- @field logger _neopilot.Logger
--- @field xid number
--- @field range _neopilot.Range?
...
function RequestContext.from_current_buffer(_neopilot, xid)
  ...
  local mds = {}
  for _, md in ipairs(_neopilot.md_files) do
    table.insert(mds, md)
  end
...
--- @param rules (_neopilot.Agents.Rule | string)[]
...
function RequestContext:finalize()
  ...
  if self.range then
    table.insert(
      self.ai_context,
      self._neopilot.prompts.get_file_location(self)
    )
    table.insert(
      self.ai_context,
      self._neopilot.prompts.get_range_text(self.range)
    )
  end
  table.insert(
    self.ai_context,
    self._neopilot.prompts.tmp_file_location(self.tmp_file)
  )
  return self
end
